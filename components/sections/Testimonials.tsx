    ...
    <section id="testimonials" className="py-24 overflow-hidden" ref={sectionRef}>
      <div className="container mx-auto px-6 sm:px-10 md:px-16 lg:px-24">
        <h2 className={`flex items-center text-2xl font-bold text-black dark:text-lightest-slate mb-8 transition-all duration-500 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          <span className="text-black dark:text-brand-accent font-mono text-xl mr-3">04.</span>
          What People Say
            ...
            <div className="swiper-wrapper items-center py-4">
              {TESTIMONIALS_DATA.map((testimonial, index) => (
                <div key={index} className="swiper-slide h-full">
                  <div className="bg-light-card dark:bg-brand-card p-6 sm:p-8 rounded-lg shadow-lg h-full flex flex-col justify-center min-h-[300px] text-left border border-zinc-300 dark:border-lightest-navy/40 transition-colors duration-300 hover:border-black dark:hover:border-brand-accent">
                    <p className="text-zinc-900 dark:text-slate italic mb-6 text-sm sm:text-base">"{testimonial.quote}"</p>
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="font-bold text-black dark:text-lightest-slate">{testimonial.name}</p>
                        <p className="font-mono text-xs sm:text-sm text-zinc-900 dark:text-slate">{testimonial.title}, {testimonial.company}</p>
                      </div>
                    </div>
                  </div>
                </div>
